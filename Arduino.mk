...
#
#       0.8+ 23.02.2106 Neil Johnson
#						Added support for ARDUINO_OPT_LIB_PATH.
#                       This allows for libs to be stored outside of
#                       Arduino install
########################################################################
#
...
#
#   ARDUINO_OPT_LIB_PATH - An additional dir to look in for Libs
#
...
ifndef ARDUINO_OPT_LIB_PATH
ARDUINO_OPT_LIB_PATH  = $(ARDUINO_DIR)/libraries
endif

ARDUINO_AVR_LIB_PATH  = $(ARDUINO_DIR)/hardware/arduino/avr/libraries
...
ifndef PARSE_BOARD
PARSE_BOARD = ./parseBoardsFile --boards_txt=$(BOARDS_TXT)
endif
...
CAT     = cat
ECHO    = echo
BLD_CSCOPE = ./bld_cscope_ardunio

# General arguments
OPT_LIBS      = $(wildcard $(patsubst %,$(ARDUINO_OPT_LIB_PATH)/%,$(ARDUINO_LIBS)))
AVR_LIBS      = $(wildcard $(patsubst %,$(ARDUINO_AVR_LIB_PATH)/%,$(ARDUINO_LIBS)))
SYS_INCLUDES  = $(patsubst %,-I%,$(OPT_LIBS)) $(patsubst %,-I%,$(AVR_LIBS))
LIB_OPT_C_SRCS    = $(wildcard $(patsubst %,%/*.c,$(OPT_LIBS)))
LIB_AVR_C_SRCS    = $(wildcard $(patsubst %,%/*.c,$(AVR_LIBS)))
LIB_OPT_CPP_SRCS  = $(wildcard $(patsubst %,%/*.cpp,$(OPT_LIBS)))
LIB_AVR_CPP_SRCS  = $(wildcard $(patsubst %,%/*.cpp,$(AVR_LIBS)))
LIB_OBJS1      = $(patsubst $(ARDUINO_OPT_LIB_PATH)/%.c,$(OBJDIR)/libs/%.o,$(LIB_OPT_C_SRCS))
LIB_OBJS2	   = $(patsubst $(ARDUINO_OPT_LIB_PATH)/%.cpp,$(OBJDIR)/libs/%.o,$(LIB_OPT_CPP_SRCS))
LIB_OBJS3	   = $(patsubst $(ARDUINO_AVR_LIB_PATH)/%.cpp,$(OBJDIR)/libs/%.o,$(LIB_AVR_CPP_SRCS))
LIB_OBJS4      = $(patsubst $(ARDUINO_AVR_LIB_PATH)/%.c,$(OBJDIR)/libs/%.o,$(LIB_AVR_C_SRCS))
LIB_OBJS 		= $(LIB_OBJS1) $(LIB_OBJS2) $(LIB_OBJS3) $(LIB_OBJS4)
CPPFLAGS      = -mmcu=$(MCU) -DF_CPU=$(F_CPU) -DARDUINO=$(ARDUINO_VERSION) \
			-I. -I$(ARDUINO_CORE_PATH) -I$(ARDUINO_VAR_PATH)/$(VARIANT) \
			$(SYS_INCLUDES) -g -Os -w -Wall \
			-ffunction-sections -fdata-sections
CFLAGS        = -std=gnu99
CXXFLAGS      = -fno-exceptions -fpermissive
ASFLAGS       = -mmcu=$(MCU) -I. -x assembler-with-cpp
LDFLAGS       = -mmcu=$(MCU) -Wl,--gc-sections,-Map,$(TARGET_MAP) -Os
...
# library sources
$(OBJDIR)/libs/%.o: $(ARDUINO_OPT_LIB_PATH)/%.c
	mkdir -p $(dir $@)
	$(CC) -c $(CPPFLAGS) $(CFLAGS) $< -o $@

$(OBJDIR)/libs/%.o: $(ARDUINO_OPT_LIB_PATH)/%.cpp
	mkdir -p $(dir $@)
	$(CC) -c $(CPPFLAGS) $(CXXFLAGS) $< -o $@

$(OBJDIR)/libs/%.o: $(ARDUINO_AVR_LIB_PATH)/%.c
	mkdir -p $(dir $@)
	$(CC) -c $(CPPFLAGS) $(CFLAGS) $< -o $@

$(OBJDIR)/libs/%.o: $(ARDUINO_AVR_LIB_PATH)/%.cpp
	mkdir -p $(dir $@)
	$(CC) -c $(CPPFLAGS) $(CXXFLAGS) $< -o $@
...
clean:
		$(REMOVE) $(LOCAL_OBJS) $(CORE_OBJS) $(LIB_OBJS) $(CORE_LIB) $(TARGETS) $(DEP_FILE) $(DEPS) $(TARGET_MAP) cscope.*
...
cscope:
	$(BLD_CSCOPE) $(ARDUINO_OPT_LIB_PATH) $(ARDUINO_AVR_LIB_PATH) $(ARDUINO_CORE_PATH)
